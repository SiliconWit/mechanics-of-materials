...
**Components:**
- `E022-01` = Course code (Mechanics of Materials)
...
1. **Draw Free Body Diagram (Equivalent System Model)**
   ...
   - Downward force P = 1200 N at right end (x = 800 mm)
...
5. **Add Load Visualization**

   - Switch to Part workbench
   - Create cylinder at free end (x=500mm) pointing downward
   - Color it red to represent 500 N force
   - Add text label 'P = 500 N' using Draft workbench

6. **Create Engineering Context**

   - Add simple gripper representation (two small boxes at free end)
   - Add simple joint representation (cylinder at fixed end)
   - Add dimension annotations showing 500mm length
...
7. **Create Technical Drawing (TechDraw Workbench)**
   ...
   - Position front view showing 500mm length and 20mm height
   - Add top view showing 60mm width
   - Add section view (A-A) through beam cross-section
   - Insert → Dimensions → Add length dimensions (500mm, 60mm, 20mm)
   - Insert → Annotation → Add material note: "Al 6061-T6"
   - Insert → Annotation → Add load note: "P = 500 N at free end"
   - File → Export as PDF → Save as per naming convention

8. **Export Model and Images**

   - Rotate to isometric view
   - View → Screenshot → Save as per naming convention (Isometric.png)
   - Rotate to side view
   - View → Screenshot → Save as per naming convention (SideView.png)
   - **Export 3D model:** File → Export → Select ".glb" format → Save as per naming convention
   - (GLB format is preferred for web visualization and is universally compatible)
...
1. **Create 3D Model of Hollow Beam**
   ...
   - Close sketch, Pad 1200 mm along length
   - Create second sketch on same face
   - Draw 34×34 mm square centered (inner cavity)
   - Close sketch, Pocket through all (creates hollow section)

2. **Switch to FEM Workbench**

   - Workbench dropdown → FEM
   - Create Analysis container: Analysis → Analysis container

3. **Add Material**

   - Analysis → Material → FEM material for solid
...
<div style={{ display: 'flex', justifyContent: 'center', width: '100%' }}>
  <TailwindWrapper>
    ![Drone Arm Structure](https://pub-b1eb899abc464b5baf5b9521d163efd9.r2.dev/solid-mechanics/quadcopter-drone-normal.png)
  </TailwindWrapper>
</div>
...
1. <Badge text="Analytical Methods" variant="success" /> - Apply equilibrium principles and stress formulas to real engineering problems
2. <Badge text="Programming Skills" variant="success" /> - Automate calculations and create parametric design studies using Python
3. <Badge text="FEM Simulation" variant="success" /> - Validate analytical solutions with finite element analysis
4. <Badge text="Design Optimization" variant="success" /> - Make informed design decisions balancing performance, weight, and cost
5. <Badge text="Technical Communication" variant="success" /> - Document engineering analysis in professional reports
...
- Apply these analysis techniques to your capstone design projects
