...
:::note[Problem Statement]
- **Loading condition:** End point load (contact force with overhead wire)
- **Support configuration:** Cantilever beam fixed at train roof connection
<div style={{ display: 'flex', justifyContent: 'center', width: '100%' }}>
  <TailwindWrapper>
...
1. **Support reaction forces** under contact loading
2. **Shear force and bending moment diagrams** across the cantilever
3. **Maximum bending stress** at the critical cantilever section
4. **Safety factor assessment** against yield failure
5. **Failure risks** during sudden contact loss scenarios

**Key Question:** Can this pantograph arm safely withstand contact forces including dynamic effects, and what are the implications of sudden load removal?
:::

<Card title="🔧 System Parameters" icon="document">
<LoadingDiagramEmbed
  src="/education/mechanics-of-materials/components/lesson-2-2/pantograph_loading_diagram.html"
...
2. **Apply equilibrium equations:**
   ...
   **Vertical force equilibrium:** $\sum F_y = 0$
   ...
   $R_A = P = 800 \text{ N}$ (upward reaction at fixed support)

3. **Moment equilibrium about fixed support A:** $\sum M_A = 0$

   $M_A - P \times L = 0$

   $M_A = 800 \times 1200 = 960,000 \text{ N·mm}$ (reaction moment at fixed support)

4. **Verify equilibrium:**

   - Vertical forces: $R_A - P = 800 - 800 = 0$ (check)
   - Moments about any point: $M_A - P \times L = 960,000 - 960,000 = 0$ (check)

</Steps>

</details>

### Step 2: Shear Force and Bending Moment Analysis

<details>
<summary>**Click to reveal shear and moment calculations**</summary>
...
2. **Verify shear force values:**
   ...
   - At free end (x = 0): V = -800 N
   - At fixed support (x = 1200 mm): V = -800 N
   - No variation along beam length for single concentrated load (check)

</Steps>

- **Calculate bending moments at critical points:**

- **Method:** For cantilever beam, moment at distance x from free end = -P × x

<Steps>
1. **At free end (x = 0):**

   $M = 0 \text{ N·mm}$ (free end boundary condition)

2. **At quarter span (x = 300 mm):**

   $M = -P \times x = -800 \times 300 = -240,000 \text{ N·mm}$

3. **At midspan (x = 600 mm):**

   $M = -P \times x = -800 \times 600 = -480,000 \text{ N·mm}$

4. **At fixed support (x = 1200 mm):**

   $M = -P \times x = -800 \times 1200 = -960,000 \text{ N·mm}$ (maximum moment)

</Steps>

> **Note:** Negative moments indicate compression on the top fiber and tension on the bottom fiber.
...
2. **Apply cantilever moment equations:**
   ...
   $M_{max} = P \times L = 800 \times 1200 = 960,000 \text{ N·mm}$ ✅

3. **Verify moment distribution:**

   - At free end (x = 0): M = 0
   - At fixed end (x = L): M = -960,000 N·mm (negative indicates compression on top fiber)
...
1. **Apply flexural formula:**
   ...
   $$\sigma_{max} = \frac{M \cdot c}{I} = \frac{960,000 \times 25}{2.45 \times 10^6} = \frac{24,000,000}{2,450,000} = 9.80 \text{ MPa}$$

2. **Alternative using section modulus:**

   $$S = \frac{I}{c} = \frac{2.45 \times 10^6}{25} = 98,000 \text{ mm}^3$$

   $$\sigma_{max} = \frac{M}{S} = \frac{960,000}{98,000} = 9.80 \text{ MPa}$$ ✅

3. **Stress distribution:**

   - Maximum tensile stress: +9.80 MPa (bottom fiber at fixed end)
   - Maximum compressive stress: -9.80 MPa (top fiber at fixed end)
   - Neutral axis stress: 0 MPa ✅
...
1. **Calculate actual safety factor:**
   ...
   $$SF_{actual} = \frac{\sigma_{yield}}{\sigma_{max}} = \frac{250}{9.80} = 25.5$$
...
### Step 6: Shear and Moment Analysis

<Card title="📊 Shear Force Diagram" icon="chart">
<ChartEmbed
  src="/education/mechanics-of-materials/components/lesson-2-2/pantograph_shear_chart.html"
  title="Pantograph Arm Shear Force Distribution"
/>

**Key observations:**
- Constant shear force V = -800 N throughout the cantilever
- Negative sign indicates downward internal force
- No variation along beam length for concentrated end load
</Card>

<Card title="📈 Bending Moment Diagram" icon="chart">
<ChartEmbed
  src="/education/mechanics-of-materials/components/lesson-2-2/pantograph_moment_chart.html"
  title="Pantograph Arm Bending Moment Distribution"
/>

**Key observations:**
- Linear moment variation: M(x) = -Px = -800x N⋅m
- Maximum moment at fixed support: M_max = -960 N⋅m
- Zero moment at free end
- Negative moment causes compression on top fiber
</Card>
...
2. **Sudden contact loss scenarios:**
   ...
   **Risk 2: Recontact impact**
   ...
   - Still within safety margin: 4 × 9.80 = 39.2 MPa $\ll$ 250 MPa ✅
