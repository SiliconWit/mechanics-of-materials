...
4. **Contact stress (localized):**
   $$\sigma_y = -50 \text{ MPa (compressive)}$$

</Steps>

### Step 2: Construct Mohr's Circle Analysis
...
2. **Circle parameters:**
   ...
   $$R = \sqrt{\left(\frac{\sigma_x - \sigma_y}{2}\right)^2 + \tau_{xy}^2} = \sqrt{\left(\frac{180.4}{2}\right)^2 + 163.2^2} = \sqrt{8136 + 26634} = 186.4 \text{ MPa}$$
...
1. **Maximum Normal Stress Theory:**
   $$\sigma_1 = 226.6 \text{ MPa} < \frac{\sigma_{yield}}{SF} = \frac{470}{4} = 117.5 \text{ MPa}$$
   **Result:** Fails this criterion! ❌

2. **Maximum Shear Stress Theory (Tresca):**
   $$\tau_{max} = 186.4 \text{ MPa} < \frac{\sigma_{yield}}{2 \cdot SF} = \frac{470}{8} = 58.75 \text{ MPa}$$
   **Result:** Fails this criterion! ❌
...
3. **von Mises Equivalent Stress:**
   ...
   $$\sigma_{eq} = 342.7 \text{ MPa} > \frac{\sigma_{yield}}{SF} = 117.5 \text{ MPa}$$
   **Result:** Fails this criterion! ❌

</Steps>

:::caution[Critical Design Assessment]
**All failure theories indicate that the current pin design is inadequate** for the applied loading conditions with the required safety factor.

The universal joint pin requires immediate redesign to prevent failure.
:::
